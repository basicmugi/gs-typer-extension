...
function load() {
  chrome.storage.local.get(["tb_enabled","tb_volume","tb_play_in_passwords","tb_include_content_editable"], (res) => {
    ...
    $("volume").value = vol; $("volv").textContent = vol.toFixed(2);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("volume").addEventListener("input", e => { const v = parseFloat(e.target.value); $("volv").textContent = v.toFixed(2); save("tb_volume", v); });
